...
%% Figures definition
f1 = figure('Name', 'Road view');
%f2 = figure('Name', 'Histogram L');
%f3 = figure('Name', 'Histogram R');
...
while hasFrame(videoReader)
    ...
    if(f==1 || f==2) %discard first two frames because sometimes they are completely white
        continue
    end
...
%     plot(right_lane_inds(1, :), right_lane_inds(2, :), 'o');
    ...
    if(abs(left_lane_inds(1, 2)-center) < warning_th)
        ...
        fprintf("Current distance of the left side of the car from the delimiter: %f \n", curr_dist_m-(car_width_m/2))
        beep
    end
    if(abs(right_lane_inds(1, 2)-center) < warning_th)
        fprintf("Warning: lane RIGHT delimiter crossing! \n") %activate warning if the car is about to cross a lane
        fprintf("Current distance of the right side of the car from the delimiter: %f \n", curr_dist_m-(car_width_m/2))
        %curr_dist_m-(car_width_m/2)
        beep
    end
